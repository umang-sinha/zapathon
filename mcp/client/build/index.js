...
const RunServer = () => {
    try {
        ...
        const getOrCreateMcpClient = async (userId) => {
            if (!McpClients.has(userId)) {
                console.log(`Creating and initializing new MCPClient for user: ${userId}`);
        ...
        app.get("/process", async (req, res) => {
            ...
            if (!userId) {
                return res.status(400).json({
                    error: "Missing 'userId' parameter. Please use: /process?query=your_message&userId=your_unique_id",
                });
            }
            try {
                const client = await getOrCreateMcpClient(userId);
                const responseContent = await client.ProcessMessage(userQuery);
        ...
        app.get("/new-chat", async (req, res) => {
            ...
            if (!userId) {
                return res.status(400).json({
                    error: "Missing 'userId' parameter. Please use: /new-chat?userId=your_unique_id",
                });
            }
            if (McpClients.has(userId)) {
